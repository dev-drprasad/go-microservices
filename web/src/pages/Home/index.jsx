...
const formatNewOrdersTooltip = (v) => [v, "orders"];

function Home() {
  const [newCustomersDaily = [], status] = useAPI("/api/v1/customers/aggregations/new");
  const [newOrdersDaily = [], newOrdersDailyStatus] = useAPI("/api/v1/orders/aggregations/new");
  return (
    <div className="home">
      ...
      <Card title="New Orders" size="small">
        <NSHandler status={newOrdersDailyStatus}>
          {() => (
            <ResponsiveContainer height={300}>
              <AreaChart data={newOrdersDaily}>
                <defs>
                  <linearGradient id="uvcolor" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#00cc83" stopOpacity={0.95} />
                    <stop offset="95%" stopColor="#FFFFFF" stopOpacity={0.1} />
                ...
                <CartesianGrid strokeDasharray="3 3" stroke="#dddddd" />
                <Area type="monotone" dataKey="count" stroke="#00cc83" strokeWidth={2} fill="url(#uvcolor)" />
                <Tooltip formatter={formatNewOrdersTooltip} labelFormatter={formatXAxis} />
                <XAxis dataKey="date" {...axisPros} tickFormatter={formatXAxis} />
                <YAxis {...axisPros} width={32} />
              </AreaChart>
            </ResponsiveContainer>
