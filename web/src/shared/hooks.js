...
export function useBrands() {
  ...
  const [patients = [], status, refresh] = useAPI(`/api/v1/brands?${queryParams}`);
...
export function useCategories() {
  ...
  const [physicians = [], status, refresh] = useAPI(`/api/v1/categories?${queryParams}`);
  return [physicians, status, refresh];
}
...
export function useProduct(id) {
  ...
  const [product, status, refresh] = useAPI(id ? `/api/v1/products/${id}` : undefined);

  const saveOpts = useMemo(() => {
    if (!payload) return [undefined];
    const [urlpath, method] = id ? [`/api/v1/products/${id}`, "PUT"] : ["/api/v1/products", "POST"];
...
export function useCustomer(id) {
  const [payload, setPayload] = useState(undefined);
  const [product, status, refresh] = useAPI(id ? `/api/v1/customers/${id}` : undefined);

  const saveOpts = useMemo(() => {
    if (!payload) return [undefined];
    const [urlpath, method] = id ? [`/api/v1/customers/${id}`, "PUT"] : ["/api/v1/customers", "POST"];
    return [urlpath, { method, body: JSON.stringify(payload) }];
  }, [payload, id]);
  const [, saveStatus] = useAPI(...saveOpts);
...
export function useOrder(id) {
  const [payload, setPayload] = useState(undefined);
  const [order, status, refresh] = useAPI(id ? `/api/v1/orders/${id}` : undefined);

  const saveOpts = useMemo(() => {
    console.log("p :>> ", payload, id);
    if (!payload) return [undefined];
    const [urlpath, method] = id ? [`/api/v1/orders/${id}`, "PUT"] : ["/api/v1/orders", "POST"];
    return [urlpath, { method, body: JSON.stringify(payload) }];
  }, [payload, id]);
  const [, saveStatus] = useAPI(...saveOpts);
